...
export async function registerInstagramRoutes(app: FastifyInstance, deps: InstagramRoutesDeps): Promise<void> {
  ...
  app.post("/instagram/posts", { schema: { body: SCHEDULE_POST_BODY_SCHEMA } }, async (request) => {
    ...
    await new PublicationWorker(orchestratorDeps, `${target}-publish-worker`).runUntilIdle(1);
  ...
  app.post("/instagram/posts/run-due", { schema: { body: { type: "object", properties: { now: { type: "string" } } } } }, async (request) => {
    ...
    const processed = await new PublicationWorker(orchestratorDeps, "meta-due-worker").runUntilIdle();
