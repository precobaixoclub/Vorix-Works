...
/**
 * Módulo Conversas — Fase 1 (Fundação). Esta tela cobre só gestão de conexão (cadastrar, parear
 * via QR, ver status, desconectar) — a Inbox de verdade (lista de conversas, timeline, envio) é a
 * Fase 3, construída sobre esta base. Ver plano técnico, "Fases".
 */
export default function ConversasPage() {
  const workspace = useCurrentWorkspace();
  const { data, isLoading, error, mutate } = useInboxConnections(workspace.id);
  ...
  async function handleCreateConnection() {
    ...
    setCreating(true);
    try {
      await createInboxConnection(workspace.id, displayName);
      setNewDisplayName("");
      await mutate();
    } finally {
      setCreating(false);
  ...
  async function handleShowQrCode(connectionId: string) {
    const { qrCode } = await getInboxConnectionQrCode(workspace.id, connectionId);
    setQrByConnectionId((prev) => ({ ...prev, [connectionId]: qrCode }));
  }

  async function handleRefreshStatus(connectionId: string) {
    await refreshInboxConnectionStatus(workspace.id, connectionId);
    await mutate();
  }

  async function handleDisconnect(connectionId: string) {
    await disconnectInboxConnection(workspace.id, connectionId);
    await mutate();
  }
  ...
  return (
    <main className="mx-auto max-w-4xl px-3 py-5 sm:px-6 sm:py-8">
      <PageHeader title="Conversas" description="Conexões de WhatsApp usadas pela central de atendimento do Vorix." />
