...
/**
 * Registro de schemas por tipo — Sprint 07 (escopo aprovado). Só `campaign_creation` tem schema
 * implementado; `campaign_edit`/`content_request`/`knowledge_query`/`asset_search`/
 * `generic_task` existem como `BriefingType` válido (o contrato existe), mas `getBriefingSchema`
 * devolve `undefined` para eles — quem chama trata a ausência como "sem schema ainda" e preserva
 * o comportamento conversacional da Sprint 06 para essa intenção, nunca cria uma implementação
 * pela metade.
 */
