...
export async function registerYouTubeRoutes(app: FastifyInstance, deps: YouTubeRoutesDeps): Promise<void> {
  ...
  app.post("/youtube/posts", { schema: { body: SCHEDULE_POST_BODY_SCHEMA } }, async (request) => {
    ...
    await new PublicationWorker(orchestratorDeps, "youtube-publish-worker").runUntilIdle(1, { tenantId: principal.tenantId, workspaceId: body.workspaceId });
  ...
  app.post("/youtube/posts/run-due", { schema: { body: { type: "object", properties: { workspaceId: { type: "string" }, now: { type: "string" } } } } }, async (request) => {
    ...
    const processed = await new PublicationWorker(orchestratorDeps, "youtube-due-worker").runUntilIdle(100, { tenantId: principal.tenantId, workspaceId: body.workspaceId });
