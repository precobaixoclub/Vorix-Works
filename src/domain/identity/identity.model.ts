...
export const PERMISSIONS = [
  ...
  "ads:sync",
  "ads:manage",
  // Módulo Conversas (Fase 1) — inbox de WhatsApp via WuzAPI e, desde a unificação do canal
  // Instagram (Fase 5 revista — o antigo módulo Instagram DM Automation standalone foi absorvido
  // aqui), também Instagram DM. `manage_connections` fica num degrau acima de `reply`/`assign`
  // porque criar/desconectar uma conexão é uma ação de infraestrutura da conta (mesmo raciocínio de
  // `credential:connect` ficar só em admin/owner); `manage_ai` também fica restrito porque decide se
  // uma automação responde clientes reais sem revisão humana.
  "inbox:read",
  "inbox:reply",
  "inbox:assign",
  ...
  "proposal:send",
  "automation:manage",
  // Agenda (réplica adaptada do CMDesk, pedido explícito do usuário) — `calendar_event:*` (nunca
  // `calendar:*`, já usado pelo calendário editorial de Publication/Scheduling acima — recursos
  // diferentes, nunca a mesma permissão). `read` liberado a todo mundo (até viewer, mesmo
  // raciocínio de `inbox:read`); `write` (criar/editar/cancelar/concluir o PRÓPRIO evento) exige
  // editor+ — ver `assertCanMutateCalendarEvent`, que ainda restringe a dono ou admin/owner.
  "calendar_event:read",
