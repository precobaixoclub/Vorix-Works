 ...
 *
 * MOTOR LEGADO — sempre usa `buildGuardedPrompt` (`legacy-pedro-image-guard.ts`), a guarda
 * completa herdada do Pedro (inclui a regra de "sem texto nenhum" quando não há `authorizedTitle`).
 * O motor GPT (`openai-creative-image-provider.ts`) nunca passa por aqui — usa sua própria classe e
 * sua própria guarda mínima, exatamente para que a escolha de guarda seja sempre explícita por qual
 * provider foi instanciado, nunca inferida em runtime (ver migração "GPT como motor criativo único").
 */
