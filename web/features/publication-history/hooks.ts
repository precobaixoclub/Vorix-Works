...
/** Junta os feeds reais de TikTok/Instagram/Facebook/Kwai numa lista só — mesma lógica que vivia
...
export function useUnifiedPublications(workspaceId: string) {
  ...
  const data = useMemo<UnifiedPublication[] | undefined>(() => {
    if (tiktok.data === undefined && meta.data === undefined && kwai.data === undefined) return undefined;
  ...
  }, [tiktok.data, meta.data, kwai.data]);

  const error = tiktok.error ?? meta.error ?? kwai.error;
  const isLoading = data === undefined && !error;

  async function mutate() {
    await Promise.all([tiktok.mutate(), meta.mutate(), kwai.mutate()]);
  }
