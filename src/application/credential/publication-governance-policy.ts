...
export class PublicationGovernancePolicy {
  ...
  decide(input: {
    ...
    if (!isSandboxExternalProvider(input.providerId) && input.providerId !== this.canaryPolicy.providerId) return { allowed: false, reason: "provider_mismatch", safeMessage: "Provider fora da governança canário configurada." };
    ...
    if (!this.canaryPolicy.tenantIds.includes(input.tenantId)) return { allowed: false, reason: "tenant_not_allowed", safeMessage: "Tenant fora do canário." };
    if (!this.canaryPolicy.workspaceIds.includes(input.workspaceId)) return { allowed: false, reason: "workspace_not_allowed", safeMessage: "Workspace fora do canário." };
    ...
    if (input.binding.status !== "active" || !input.binding.canary) return { allowed: false, reason: "credential_inactive", safeMessage: "Binding da credencial não está ativo no canário." };
